USE [master];
GO
IF(SELECT OBJECT_ID('sp_GenMerge', 'P')) IS NOT NULL
    DROP PROC sp_GenMerge;
GO
/***********************************************************************************************************************************************
Autor:		 Micha³ Go³oœ http://toolien.pl/sp_genmerge/
...
END;
GO
--Mark the procedure as a system object to allow it to be called from other databases
EXEC sp_MS_marksystemobject sp_GenMerge;
GO